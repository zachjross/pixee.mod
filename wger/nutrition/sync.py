...
def fetch_image_from_wger_instance(ingredient):
    ...
    result = safe_requests.get(url, headers=wger_headers()).json()
    if result['count'] == 0:
    ...
    try:
        ...
        retrieved_image = safe_requests.get(image_data['image'], headers=wger_headers())
...
def fetch_image_from_off(ingredient):
    ...
    product_data = safe_requests.get(url, headers=headers).json()
    ...
    response = safe_requests.get(image_url, headers=headers)
